...
async function loadFonts() {
  try {
    if (!fontSemibold) {
      ...
      console.log('Loading Semibold font from:', semiboldUrl);
      fontSemibold = await opentype.load(semiboldUrl);
      console.log('Semibold font loaded:', fontSemibold);
    }
    if (!fontMedium) {
      const mediumUrl = new URL('../fonts/MiSans-Medium.ttf', import.meta.url).href;
      console.log('Loading Medium font from:', mediumUrl);
      fontMedium = await opentype.load(mediumUrl);
      console.log('Medium font loaded:', fontMedium);
    }
    if (!fontNormal) {
      const normalUrl = new URL('../fonts/MiSans-Normal.ttf', import.meta.url).href;
      console.log('Loading Normal font from:', normalUrl);
      fontNormal = await opentype.load(normalUrl);
      console.log('Normal font loaded:', fontNormal);
    }
...
function buildExifObj(exifData, piexif) {
  ...
  if (exifData.FocalLength) {
    // 支持 "50mm" 或 "50" 格式
    const focalStr = String(exifData.FocalLength).replace(/mm$/i, '');
  ...
  if (exifData.FNumber) {
    // 支持 "f/1.8"、"F1.8" 或 "1.8" 格式
    const fNumStr = String(exifData.FNumber).replace(/^f\//i, '').replace(/^F/, '');
    const fNum = parseFloat(fNumStr);
    if (!isNaN(fNum)) {
